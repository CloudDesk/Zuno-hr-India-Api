...
**Note:** Restricted holidays are NOT included here (handled separately via Leave model with leaveType: 'restricted_holiday')
...
**Impact:** 
- Mandatory holidays included in `payableDays`
- Counted separately from leaves
- Restricted holidays handled via Leave model (leaveType: 'restricted_holiday')
...
- `presentDays` - Days with attendance (excluding weekends)
- `weekendDays` - Weekend days in the month
- `holidayDays` - Mandatory holidays + approved restricted holidays
- `approvedLeaves` - Approved leave days (annual + compOff, supports decimals)
- `LOPDays` - Loss of Pay days (absent without leave)

**Formulas:**
```
payableDays = presentDays + weekendDays + holidayDays + approvedLeaves
LOPDays = max(0, totalDaysInMonth - payableDays)
...
- Restricted holidays are handled via Leave model (leaveType: 'restricted_holiday')
- Half-day leaves are supported (decimal `noOfDays`)
- `LOPDays` can be 0 even if `payableDays > daysInMonth` (due to half-day leaves)
...
### 5. **Holiday Handling**
- **Mandatory holidays:** Included in `payableDays` (from HolidayCalendar)
- **Restricted holidays:** Handled via Leave model (leaveType: 'restricted_holiday'), included in `holidayDays` if approved
- **Optional holidays:** Not automatically included (requires leave application)
...
payableDays = presentDays 
            ...
            + holidayDays (mandatory + approved restricted)
            + approvedLeaves (annual + comp-off, supports decimals)
```
...
### 11. **Restricted Holiday Handling**
- Restricted holidays are counted in `holidayDays`, NOT in `approvedLeaves`
- Only APPROVED restricted holidays count as holidays
- If not applied/approved, they are working days
- Treated as paid holidays, not leaves
...
### Q4: Are restricted holidays counted as leaves?
**A:** No. Restricted holidays are counted in `holidayDays`, not in `approvedLeaves`. They're treated as paid holidays, not leaves.
...
- **File:** `src/services/payroll.service.ts`
  ...
  - `fetchApprovedLeaves()` - Get approved leaves (Lines 1747-1767)
